        ...
        <div className="p-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-widest text-white/40 mt-1 font-semibold">Pro Dashboard</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 flex flex-col gap-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
          ...
          {navItems.map((item) => (
            item.isLocked ? (
              <div
                key={item.path}
                className="flex items-center justify-between px-4 py-2.5 rounded-xl text-white/30 select-none group"
              >
                <div className="flex items-center gap-3">
                  <span className="text-lg opacity-40 grayscale">{item.icon}</span>
                  <span className="text-sm font-medium">{item.label}</span>
                </div>
                <svg className="w-3.5 h-3.5 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
